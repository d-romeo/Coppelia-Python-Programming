...
franka_dh_params = [
        ...
        {'a': 0,       'alpha':        0, 'd': 0.172, 'theta': 0},  # q7
    ]
# 65 mm per end effector
...
def real_geometric_jacobian(kinematic_chain:list, q:list) -> np.ndarray:
    p_ee = kinematic_chain[-1][0:3, 3]   # Reale o aumentato
...
def arm_error_minimization_inverse_kinematics(dh_params_robot:list, dh_params_human:list, q_h_des:np.ndarray, 
                                              qh_start:list, qr_first_guess:list, base_world_transform:np.ndarray, q0_W: np.ndarray,  
                                              conv_thresh:float=0.001, max_iterations:int=1e4, damping_factor:float=0.001, 
                                              step_size:float=0.05, verbose:bool=True):
    
    q_human = np.array(qh_start)
    q_robot = np.array(qr_first_guess)
    ...
    while not stop:
        kin_chain_robot = forward_kinematics_panda(dh_params=dh_params_robot, q=q_robot, base_world_transform=base_world_transform)
        kin_chain_human = forward_kinematics(dh_params=dh_params_human, q=q_human, base_world_transform=q0_W)

        J_robot = geometric_jacobian(kinematic_chain=kin_chain_robot, q=q_robot.tolist())
        J_human = real_geometric_jacobian(kinematic_chain=kin_chain_human, q=q_human.tolist())

        J_pinv_robot = J_robot.T @ np.linalg.inv(J_robot@J_robot.T + damping_factor*np.eye(J_robot.shape[0]))
        J_pinv_human = J_human.T @ np.linalg.inv(J_human@J_human.T + damping_factor*np.eye(J_human.shape[0]))

        e_qr = q_h_des - q_human
        #print(e_qr)
        q_robot_old = q_robot.copy()
        q_robot = q_robot_old + J_pinv_robot@J_human@e_qr 
        q_human = q_human + J_pinv_human@J_robot@(q_robot - q_robot_old) 

        success = np.linalg.norm(e_qr)<conv_thresh
        stop = (it == max_iterations) or success

       #if stop == True: 
            #print(f"[INFO] primo q_robot {qr_first_guess}") 
           # print(f"[INFO] ultimo q_robot {q_robot}") 

        if it%100 ==0 and verbose: 
            print(f"It: {it} Err:{np.linalg.norm(e_qr):.5}" )
        it += 1

    return success, q_robot.tolist(), q_human.tolist()